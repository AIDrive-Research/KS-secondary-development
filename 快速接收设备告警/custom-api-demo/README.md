...
此类名、方法名等框架是固定的，不可修改。你可以通过实现回调方法的具体逻辑（如 `send_alert_callback` 和 `send_alert_video_callback`）以及配置类的属性（如 `ignore_alert` 等）来实现具体功能。
...
`Api` 类**必须**包含以下两个方法，用于发送检测结果、告警信息和告警视频。可根据需求实现具体的回调方法。
...
2. **方法实现**：默认情况下，`send_alert_callback` 和 `send_alert_video_callback` 方法是空方法。在实际使用中，需要根据具体需求实现这些方法的逻辑，例如将数据发送到服务器。
