...
function init() {
  Promise.all([ZQL_apis.getDevices(), ZQL_apis.getSources()]).then(res => {
    ...
    for (let deviceId in res[1].data) {
      let sourceList = res[1].data[deviceId];
      for(let i = 0; i < sourceList.length; i++){
        let sourceId = sourceList[i].id;
        sourceList[i].sourceId = sourceId;
        sourceList[i].deviceId = deviceId; 
      }
    ...
    for (let i = 0; i < devices.length; i++) {
      ...
      if (res[1].data[devices[i].id]) {
        devices[i].children = Object.values(res[1].data[devices[i].id]).map(item => {
          ...
          item.type = 'source';
          item.checked = false;
          item.port_info = devices[i].port_info;
    ...
    Object.keys(devices).forEach(devId => {
      ZQL_multivideo.connectMqtt(devices[devId].port_info.websocket)
    });    
  }).catch(err => {
    console.log(err)
  })
